...
mton.begin(9600);
   //-------------------
    ...
    //clear the screen
    lcd.print("Welcome to MK");  //displaying the message on LCD
...
if (! rtc.begin())
  ...
  // rtc.adjust(DateTime(2020,9,10, 16,00, 00));   // to renter time *************************************************************
...
void loop()                     // run over and over again   //fun2 registration
{
   ...
   if(flagPassword==0)
  {
   ...
   keypad.getKey();
     ...
     showOnLCDGeneral("am in ","loop Verification");
      ...
      rollNo=takeAttendance();
       Serial.println(" Received from Roll num from takeAttendance ");
   ...
   if(rollNo!=-1)
   { 
     sendValues(rollNo,dhm1);  //void sendValues(int float temp1,int range1)
   ...
   {
    Serial.println("Value was -1 so not shared");
   }
...
int loop_verification()
{
     ...
     int id1=getFingerprintIDez(); //don't ned to run this at full speed.
     delay(50);
