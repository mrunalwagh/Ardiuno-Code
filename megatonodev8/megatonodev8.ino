...
//-----------------------
    ...
    lcd.clear();       ////display 9
     mySerial.begin(9600);
    //clear the screen
    lcd.print("  Grapes IoT V8");  //displaying the message on LCD
    delay(1000);
  ...
  else if(range==0)
{
  ...
  Serial.println(t);
   ...
   //send h an range to node mcu
     ...
     sendValues(t,range);  //void sendValues(int float temp1,int range1)
...
void sendValues(float temp1,int range1)
{
