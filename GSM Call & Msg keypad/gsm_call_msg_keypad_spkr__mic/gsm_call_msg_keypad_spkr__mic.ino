  ...
  { 
    ...
    if(key)
    {
      if(key=='C')
      {
        ...
        lcd.print(number);
        Serial12.print("ATD");
        Serial12.print(number);
        Serial12.println(";");
        long stime=millis()+5000;
        int ans=1;
        while(ans==1)
        {           
          while(Serial12.available()>0)
          {
            if(Serial12.find("OK"))
            {
              lcd.clear();
...
void gsm_init()
{
  ...
  while(net_flag)
  {
    Serial12.println("AT+CPIN?");
    while(Serial12.available()>0)
    {
      if(Serial12.find("+CPIN: READY"))
      net_flag=0;
    }
    delay(1000);
  }
